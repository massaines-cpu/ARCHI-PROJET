...
import { List, RotateCcw, Trash2 } from "lucide-react";
import { getContagionLevelColor } from "../tools/contagionLevel";
import { deleteCase } from "../api/api";

const ListOfCases = ({ cases = [], infections = [], onCaseDeleted }) => {
      ...
      <TableWrapper>
        <Table>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <EmptyCell colSpan={5}>Aucun résultat</EmptyCell>
              </tr>
            ) : (
              filtered.map((c, i) => (
                <Row key={c.id}>
                  ...
                  <TdAction>
                    {confirmId === c.id ? (
                      <ConfirmRow>
                        <ConfirmText>Supprimer ?</ConfirmText>
                        <OuiBtn onClick={() => handleDelete(c.id)} disabled={deletingId === c.id}>
                          {deletingId === c.id ? "…" : "Oui"}
                        </OuiBtn>
                        <NonBtn onClick={() => setConfirmId(null)}>Non</NonBtn>
                      </ConfirmRow>
                    ) : (
                      <DeleteBtn onClick={() => setConfirmId(c.id)}>
                        <Trash2 size={14} />
                      </DeleteBtn>
                    )}
                  </TdAction>
...
const OuiBtn = styled.button`
  ...
  cursor: pointer;
  transition: background 0.15s;
  &:hover:not(:disabled) { background: rgba(231, 76, 60, 0.35); }
  &:disabled { opacity: 0.5; cursor: not-allowed; }
`;

const NonBtn = styled.button`
  font-family: 'Inter', sans-serif;
  font-size: 11px;
  font-weight: 500;
  padding: 3px 8px;
